...
# Función para cargar las matrices desde archivos de texto
def load_matrices_from_files(folder):
    matrices = []
...
# Cargar las matrices de las iteraciones desde archivos de texto
iteration_matrices = load_matrices_from_files(iteration_matrices_folder)
...
while True:
    ...
    # Calcular la matriz de la iteración actual
    ...
    # Cargar la matriz correspondiente a esta iteración desde el archivo
    if iteration_count < len(iteration_matrices):
        ground_truth_matrix = iteration_matrices[iteration_count]
    else:
        # Si no hay más matrices disponibles, usar una matriz vacía
        ground_truth_matrix = np.zeros((20, 20), dtype=int)

    # Comparar las matrices y construir la matriz de confusión
    confusion_matrix = np.zeros((2, 2), dtype=int)
    for i in range(20):
        for j in range(20):
            if matrix[i, j] == 1 and ground_truth_matrix[i, j] == 1:
                confusion_matrix[1, 1] += 1  # Verdadero positivo
            elif matrix[i, j] == 1 and ground_truth_matrix[i, j] == 0:
                confusion_matrix[1, 0] += 1  # Falso positivo
            elif matrix[i, j] == 0 and ground_truth_matrix[i, j] == 1:
                confusion_matrix[0, 1] += 1  # Falso negativo
            else:
                confusion_matrix[0, 0] += 1  # Verdadero negativo

    # Mostrar la matriz de confusión
    print(f"Iteración {iteration_count}:")
    print(confusion_matrix)

    # Escribir la matriz de confusión en un archivo de texto
    confusion_matrix_file = os.path.join(output_folder, f"confusion_matrix_{iteration_count}.txt")
    np.savetxt(confusion_matrix_file, confusion_matrix, fmt='%d')
