...
# Función para dividir la imagen en una matriz 20x20
...
while True:
    ...
    if old_gray is None or cv2.getTickCount() % 50 == 0:
        ...
        if p1 is not None:
            ...
            # Dibujar los vectores de movimiento y detectar celdas con magnitud mayor a 7
            for i, (new, old) in enumerate(zip(good_new, good_old)):
                ...
                if magnitude > 0.5:  # Ajusta el umbral según sea necesario
                    color = (0, 255, 0) if magnitude <= 5 else (255, 0, 0)  # Verde si la magnitud es <= 7, azul en otro caso
    ...
    # Dividir la imagen en una matriz 20x20
    divided_cells = divide_image(frame)

    # Dibujar la cuadrícula en cada celda
    for i in range(1, 20):
        cv2.line(frame, (0, i * cell_height), (width, i * cell_height), (0, 255, 255), 1)
    for j in range(1, 20):
        cv2.line(frame, (j * cell_width, 0), (j * cell_width, height), (0, 255, 255), 1)
...
# Función para dibujar la cuadrícula en cada celda
def draw_grid(image):
    for y in range(0, height, cell_height):
...
while True:
    ...
    # Dibujar la cuadrícula en el cuadro actual
    frame_with_grid = draw_grid(frame)

    # Escribir el cuadro en el video de salida
    output_video.write(frame_with_grid)

    # Mostrar la imagen original
    cv2.imshow(window, frame_with_grid)
